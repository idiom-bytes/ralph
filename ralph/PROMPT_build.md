You are executing ONE task from the implementation plan. Follow this protocol exactly.
...
0. ORIENT
   Read the files `ralph/RALPH.md` and `ralph/IMPLEMENTATION_PLAN.md`.
   Explore the codebase to understand the current state.

1. SELECT
   Pick the highest-priority incomplete task (3rd-level unchecked `- [ ]`) from `ralph/IMPLEMENTATION_PLAN.md`.
...
3. IMPLEMENT
   ...
   If functionality is missing, add it per the project goal in `ralph/RALPH.md`.

4. VERIFY
   For EACH acceptance criterion under the task, do the following:
   a. Run the specific check that proves this criterion is met.
   b. Confirm it passes.
   c. Check off the criterion in `ralph/IMPLEMENTATION_PLAN.md`.
   Then run ALL build/test/lint commands from `ralph/RALPH.md`.
   Do not check off the task until every criterion AND every mechanical check passes.

5. UPDATE
   Check off the task in `ralph/IMPLEMENTATION_PLAN.md`.
...
ONGOING RULES:
- Keep `ralph/RALPH.md` operational and brief (~60 lines). Status updates go in `ralph/IMPLEMENTATION_PLAN.md`.
- When you learn something new about how to build/run the project, update `ralph/RALPH.md`.
- When `ralph/IMPLEMENTATION_PLAN.md` grows large, clean out completed items.
- If tests unrelated to your task fail, fix them as part of this iteration.
- If you find inconsistencies between code and `ralph/RALPH.md`, update it.
